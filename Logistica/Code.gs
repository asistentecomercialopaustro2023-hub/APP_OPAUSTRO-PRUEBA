...
// Sheet datos logística: 1yQDKAqIA9H8b53aMYklYxnNQ6eOVNjouFmpAia4A2r4
// =========================================================
...
var DATA_SHEET_ID_DEFAULT = '1yQDKAqIA9H8b53aMYklYxnNQ6eOVNjouFmpAia4A2r4';
...
// ============================================================
function getDataSS() {
  var id = PropertiesService.getScriptProperties().getProperty('DATA_SHEET_ID') || DATA_SHEET_ID_DEFAULT;
...
function doGet(e) {
  try {
    ...
    if (action === 'getUsuarios') {
      var apiUsers = requireApiKey_(p);
      if (apiUsers.error) return jsonResponse(apiUsers);
      return jsonResponse({ ok: true, usuarios: getUsuarios() });
    }
    ...
    // Compatibilidad con el frontend actual de logística.
    // Este endpoint es de lectura y no expone credenciales.
    if (action === 'getAll') {
      var api = requireApiKey_(p);
      if (api.error) return jsonResponse(api);
      ensureSheets();
      return jsonResponse({
        ok: true,
        db: { equipos: getEquipos(), movimientos: getMovimientos(), mantenimientos: getMantenimientos() },
        usuarios: getUsuariosSafe_()
      });
    }
...
function doPost(e) {
  try {
    ...
    if (sess.error) {
      if (!allowFrontendWrite_() || requireApiKey_(payload).error) return jsonResponse({ ok: false, error: sess.error });
      sess = { rol: 'logistica', puedeEscribir: true, nombre: 'Modulo Logistica' };
    }
...
function getUsuariosSafe_() {
  try { return getUsuarios(); } catch (e) { return []; }
}

function allowFrontendWrite_() {
  var value = PropertiesService.getScriptProperties().getProperty('LOGISTICA_PUBLIC_WRITE');
  return value !== 'false';
}

function requireApiKey_(params) {
  var expected = PropertiesService.getScriptProperties().getProperty('LOGISTICA_API_KEY');
  if (!expected) return { ok: true };
  var received = String((params && (params.apiKey || params.key)) || '').trim();
  if (received && received === expected) return { ok: true };
...
function configurarSistema() {
  ...
  if (!dataId) {
    props.setProperty('DATA_SHEET_ID', DATA_SHEET_ID_DEFAULT);
    Logger.log('✓ DATA_SHEET_ID configurado: ' + DATA_SHEET_ID_DEFAULT);
    Logger.log('  (Datos de cabinets se guardarán en el sheet de logística)');
...
function ensureHeaderRow_(sheet, headers) {
  ...
  var current = sheet.getRange(1, 1, 1, width).getDisplayValues()[0];
  var changed = false;
  for (var i = 0; i < headers.length; i++) {
    var value = String(current[i] || '').trim();
    if (!value || /^Unnamed/i.test(value)) {
      current[i] = headers[i];
      changed = true;
    }
  }
